...
class Catcher():
    def __init__(self, initial_state) -> None:
        # ----------------------------------------
        # 机器人仿真环境变量
        self.env, self.robot = scene.swift_scene()  # 添加环境和机器人
    ...
    def robot_sim(self):
        ...
        # 球运动仿真
        self.ball.T = sm.SE3(self.predictor.get_position_at_time(self.sim_t))  # 将位置数据变换为齐次变换矩阵
    ...
    def robot_sim_compliance(self):
        ...
        self.end_cy.T = self.robot.fkine(self.robot.q) @ self.end_cy_T
        # 球运动仿真
        self.ball.T = sm.SE3(self.end_cy.T[:3, -1])  # 将位置数据变换为齐次变换矩阵
