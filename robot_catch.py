...
from POC.poc_compliant_vel_track import Vel_tracking
...
class Catcher():
    ...
    def __init__(self, initial_state, measure_stat, self_collision, ground_collision) -> None:
        ...
        self.kf = KalmanFilter3D(self.delta_t, KD, Q, R, measure_state)
        ...
        self.poc_delta_t = 0.05

        # --- Joint Space Trajectory Planner ---
        joint_max_vel = [2.5, 2.5, 2.1750, 2.1750, 2.1750, 2.1750, 2.6100, 2.6100, 2.6100]
    ...
    def get_desired_state(self, measure_value):
        ...
        print("high_level_planner:", "q_ca=", self.q_f_opt, "t_ca=", self.t_opt)

    def get_prc_joint_traj(self):
        """
        ...
        self.prc_steps, self.joint_traj, self.joint_vel_traj, _ = self.joint_planner.get_trajectory(
            self.robot.q, self.q_f_opt, self.t_opt, delta_t=0.01, lambda_param=1.1
        )

    def get_poc_compliant_vel(self):
        """
    ...
    def robot_sim_poc(self):
        ...
        self.env.step(self.poc_delta_t)
        self.sim_t += self.poc_delta_t
